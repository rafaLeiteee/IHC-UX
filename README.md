# IHC-UX

- Luís Felipe Salgado Mitidieri: 158555
- Felipe Bagnatori Rolim: 133612
- Matheus Felipe Rodrigues: 093943
- Rafael Nascimento do Carmo Leite: 140776

### Natureza do Projeto
<p align="justify">Este projeto, desenvolvido no âmbito da disciplina de Interação Humano-Computador e User Experience, visa a concepção de uma plataforma de e-commerce especializada em jogos de tabuleiro e artigos de RPG (Role-Playing Games).
O mercado de jogos analógicos tem experimentado um crescimento significativo, atraindo um público diverso e engajado. No entanto, as plataformas digitais existentes frequentemente falham em proporcionar uma experiência de usuário satisfatória. A principal lacuna identificada é a dificuldade que os consumidores enfrentam em duas frentes principais: a obtenção de informações claras e detalhadas sobre os produtos e a falta de acessibilidade durante o processo de compra. Muitos sites atuais são visualmente poluídos, de navegação complexa e não atendem às necessidades de usuários com deficiências (visuais, motoras, etc.), tornando a jornada de descoberta e aquisição de um novo jogo uma tarefa frustrante.</p>
  
<p align="justify">Nossa solução proposta é o desenvolvimento de um protótipo de e-commerce que seja simples, intuitivo e acessível. O sistema será projetado para que qualquer usuário, desde o entusiasta até o novato, possa encontrar facilmente o que procura, entender as características de um jogo (como número de jogadores, tempo de partida, complexidade, regras gerais) e finalizar a compra sem barreiras. O contexto de uso primário será a busca por um novo jogo ou artigo de RPG, seja para compra pessoal ou para presentear. Espera-se que o sistema seja utilizado em computadores desktop e dispositivos móveis.</p>

### Proto-persona
A fim de proporcionar um ponto de partida para o entendimento dos requisitos necessários neste projeto, desenvolve uma proto-persona para o usuário final. Esta pessoal fictícia representa uma hipótese das características médias da pessoa que irá experienciar este site.

#### Proto-persona: Tiago Ned
- **Idade aproximada**<p align="justify">Entre 25 a 30 anos</p>
  
- **Ocupação/estudo**<p align="justify">Graduando em STEM (Science, Technology, Engineering, Mathematics)</p>
  
- **Contexto de uso (onde/como interage com o sistema)**<p align="justify">Geralmente interage com o sistema em seu notebook, em casa ou nos intervalos das aulas na universidade. Busca por novidades, compara preços e procura por jogos que possa jogar com seus amigos nos finais de semana. Ocasionalmente, ela também acessa o e-commerce pelo celular para mostrar um jogo a um amigo ou para verificar o status de um pedido.</p>
  
- **Principais objetivos (o que deseja alcançar)**
  - Achar um novo jogo para jogar com os amigos
  - Adquirir novos jogos para sua coleção, aproveitando promoções e bons preços.
  - Manter-se atualizada sobre os lançamentos do mercado de jogos de tabuleiro e RPG.
  - Encontrar informações detalhadas sobre os jogos, como regras, número de jogadores, tempo de partida e avaliações de outros jogadores.

- **Principais dores/dificuldades:**
  - Dificuldade em encontrar coisas novas: Novos produtos costumam demorar para lançar, e produtos antigos dominam o mercado
  - Falta de informação sobre os produtos: Dificilmente os produtos possuem uma descrição clara do que ele entregará, visto que é um produto sensível à plágio.

#### Métodos de pesquisa
A proto-persona é proposta com a finalidade de ter suas opções, dificuldades e qualidades confrontadas com dados reais. Para isto, propôs-se duas pesquisas distintas:

- **Questionário Online:**<p align="justify">Foco em entender o comportamento do cliente e os produtos desejados por ele. Este método foi escolhido tendo em vista a possibilidade de obter respostas relevantes para o negócio dentro da Unifesp, uma vez que muitos alunos se enquadram na proto-persona.</p>
  
- **Análise de Sistemas Concorrentes:**<p align="justify">Foco em observar pontos fortes e fracos na usabilidade e acessibilidade dos sistemas concorrentes. Este método foi escolhido por causa da possibilidade de se obter diferenciais relevantes para o sistema, visto que existem diversos e-commerces disponíveis.</p>

## P1: User Research

### Justificativas:
#### **Análise de sistemas concorrentes:** 
<p align="justify">A análise de e-commerces concorrentes, tanto diretos (outras lojas de board games/RPG) quanto indiretos (grandes marketplaces como Amazon), nos permite identificar padrões de design, funcionalidades comuns, lacunas e pontos de atrito. Este método é ideal para esta fase inicial, pois ajuda a responder as perguntas fundamentais:</p>

- Como os concorrentes apresentam as informações dos jogos?
- Quais filtros de busca eles oferecem?
- Quão acessíveis são seus processos de navegação e checkout?
- Onde eles falham em atender às necessidades dos usuários?

<p align="justify">Ao mapear as forças e fraquezas dos sistemas existentes, podemos validar (ou rejeitar) a hipótese de que a experiência do usuário atual é deficiente, além de coletar insights valiosos para garantir que a solução proposta seja genuinamente inovadora e relevante.</p>

#### **Questionário Online:**
<p align="justify">Enquanto a análise de concorrentes nos mostra o que está sendo feito, o questionário online nos revela o que os usuários pensam e sentem a respeito.
Este método é extremamente eficaz para validar a proto-persona e a hipótese inicial em escala. Ele permite alcançar um número significativo de pessoas dentro do nosso público-alvo (entusiastas de jogos e RPG) de forma rápida e eficiente, apoiando para:</p>

- **Confirmar a Hipótese:** <p align="justify">Perguntar diretamente sobre as dificuldades que os usuários enfrentam ao comprar jogos online, validando se os problemas de "falta de informação" e "dificuldade em encontrar informações" são reais e generalizados.</p>
- **Validar a Proto-Persona:** <p align="justify">Coletar dados demográficos (idade, frequência de jogo), comportamentais (onde costumam comprar, o que mais valorizam em um site) e pessoais (suas motivações e frustrações). Isso transforma a "suposição" de persona em um perfil baseado em dados reais.</p>
- **Priorizar Funcionalidades:** <p align="justify">Perguntar o que eles consideram mais importante em um e-commerce de jogos.</p>

<p align="justify">Desta forma, o uso dos métodos de pesquisa adotados para este projeto fornecem uma ampla cobertura no contexto de levantamento de informações, uma vez que possibilitará o entendimento do que é feito hoje, e de como o usuário final interage com o atual.</p>

A pesquisa realizada pode ser acessada através do link:
- https://docs.google.com/forms/d/e/1FAIpQLSdLTwuQYwI0D0eSfPrh32THJZjGDareRMdl7IPljkSDU4B6rw/viewform

### Descobertas:
#### **Questionário online:** 
- **Reais principais dificuldades:**
<p align="justify">Dentre os problemas levantados pelos usuários finais, destacou-se a navegabilidade ruim nos sites nichados, e a falta de clareza sobre o produto final.
Ou seja, o usuário possui dificuldade em entender a experiência que terá com um jogo apenas pelo anúncio que vê, tendo que comprar apenas jogos que já conheçe ou já tenha jogado anteriormente.</p>
  
- **Validações da proto-persona:**
<p align="justify">Dentre as respostas, observou-se que a média da idade das pessoas que já compraram algum jogo de tabuleiro online é de 25 anos, o que confirma nossa hipótese inicial.
Além disso, ao observar as maiores dificuldades encontradas pelos usuários ao comprar jogos de tabuleiro de forma online, destacam-se duas principais: dificuldade em encontrar o produto desejado (navegabilidade ruim) e a falta de informações claras sobre o produto (componentes e regras).
Além disso, a pesquisa revelou que a compra dos jogos de tabuleiro se dá por pessoas que se auto consideram casuais, experientes ou entusiastas no assunto, demonstrando que os iniciantes nunca compraram e revelando o caráter de nicho do mercado.
Pode-se traçar uma relação entre a dificuldade em entender a experiência que o usuário terá com o jogo pelos anúncios e a ausência de compras por pessoas iniciantes, uma vez que não conhecem sobre o produto anunciado.
Na esfera de acessibilidade, foi relatado também a dificuldade visual em reconhecer os elementos da tela, e sendo sugerido o alto contraste e o aumento das letras.</p>
  
- **Funcionalidades relevantes:**
Desta forma, destaca-se como funcionalidades relevantes:
- A fácil navegabilidade
- A opção de alto contraste e letras grandes
- A fácil identificação do produto (regras, tamanho, duração e recomendação de categoria adequada)

#### **Análise de sistemas concorrentes:**

- **Contextualização**

<p align="justify">Embora os jogos de tabuleiro modernos tenham crescido em popularidade, eles ainda são considerados um produto de nicho. Por esse motivo, é muito raro encontrar uma loja dedicada exclusivamente a eles. Geralmente, esses jogos ou são vendidos em canais de varejo comuns, ou em estabelecimentos especializados em produtos da cultura geek, que agregam itens como RPGs (Dungeons & Dragons), card games (Magic: The Gathering), quadrinhos e mangás, oferecendo um portfólio diversificado para esse público específico.

Para o estudo de caso desse projeto, escolhemos a loja de RPG Devir como experimento.</p>

- **Devir**

<p align="justify">Fundada em 1987 em São Paulo, a Devir foca na criação de universos de ficção, tornamos acessíveis movimentos culturais que não existiam fora dos países onde foram originalmente criados; procuramos desenvolver os mercados da imaginação e tudo o que diz respeito à fantasia, aventura, ficção científica, jogos e quadrinhos. Somos uma empresa de edição e distribuição de jogos e de publicações para os mercados português, brasileiro e espanhol. Especializados em traduções e adaptações de muitos produtos diferentes, focamo-nos no chamado entretenimento inteligente: jogos de tabuleiros modernos, jogos de RPG, jogos de cartas colecionáveis e de miniaturas, livros e HQs.</p>

Retirado de: https://devir.com.br/quem-somos/

- **Acesso**

<p align="justify">As listagens de produtos se encontravam em um dropdown. Essa abordagem consiste em um menu suspenso que, ao ser clicado, revela uma lista com todas as categorias ou itens disponíveis para o usuário escolher. Embora esta não seja mais a prática mais comum para navegação principal em sites complexos (em muitos e-commerces e sistemas antigos esta prática ainda é utilizada), devido a questões de usabilidade, é uma solução simples e amplamente implementada para organizar a oferta de mercadorias em um espaço limitado da tela.</p>

<p align="justify">Um ponto positivo significativo dessa estrutura é que todos os produtos estavam em um único lugar. Isso facilita, em certa medida, o trabalho de gestão e atualização do catálogo. Para lojas com um portfólio pequeno e bem definido, essa centralização torna a navegação direta e descomplicada, permitindo que o cliente visse o escopo completo da oferta sem a necessidade de percorrer várias páginas ou seções diferentes do site.</p>


<p align="center">
  <img width="339" height="320" alt="devir1" src="https://github.com/user-attachments/assets/d992b86f-280c-496f-b555-8ef9dd2ff103" />
  <br><em>Figura 1 - Dropdown de Produtos do Site</em>
</p>


- **Listagem de Produtos**

<p align="justify">A navegação é confusa na página principal dos produtos relacionados a jogos de tabuleiro, visto que tais produtos são listados em ordem de lançamento ao site. A escolha por esta organização ao invés de categorizar os itens por tipo, função ou popularidade, exige que o usuário percorra uma sequência interminável de itens novos e antigos sem uma lógica aparente. Para encontrar algo específico, o cliente é forçado a uma busca manual e demorada, "caçando" visualmente entre dezenas ou centenas de entradas, o que transforma uma simples tarefa de compra em uma experiência frustrante e ineficiente.</p>

<p align="center">
  <img width="1206" height="786" alt="devir2" src="https://github.com/user-attachments/assets/5b5f2f7c-5c13-4537-a9ef-232de4599ea8" />
  <br><em>Figura 2 - Página Principal dos Jogos de Tabuleiro</em>
</p>

<p align="justify">Outro grande empecilho para a usabilidade é o fato de não haver um sistema de busca para um produto específico. A ausência de uma barra de pesquisa força todos os usuários a depender exclusivamente da navegação por listagens, que, como já mencionado, é desorganizada. Se um cliente chega ao site sabendo exatamente o que quer, ele não tem um caminho direto para alcançar seu objetivo. Essa falta de um atalho essencial não apenas aumenta significativamente o tempo para encontrar um item, mas também eleva drasticamente a taxa de desistência, pois os usuários tendem a abandonar a plataforma pelo cansaço.</p>

<p align="center">
  <img width="1224" height="705" alt="devir3" src="https://github.com/user-attachments/assets/a7c6269b-a9fb-4386-8cc8-df647645346a" />
  <br><em>Figura 3 - Ordenação dos Produtos</em>
</p>

- **Detalhes de Produto Específico**

<p align="justify">A página principal de um produto específico o detalha em forma de artigo, exibindo imagens do conteúdo do jogo, além de creditar o autor. Essa abordagem, que vai além de uma simples ficha técnica, enriquece a experiência do usuário, permitindo que ele se informe e se envolva com o produto de maneira mais narrativa e visual. </p>

<p align="center">
  <img width="719" height="828" alt="devir4" src="https://github.com/user-attachments/assets/a5ab24f6-7037-488d-8e6d-6bf1e41ebbdf" />
  <br><em>Figura 4 - Detalhamento do Produto 'Transgalactica'</em>
</p>

<p align="center">
  <img width="719" height="761" alt="devir5" src="https://github.com/user-attachments/assets/8225e7ef-f1dd-42cb-8b55-5c8b8471f379" />
  <br><em>Figura 5 - Especificações do Produto 'Transgalactica'</em>
</p>


<p align="justify">Esse aspecto é ainda mais valorizado pelo fato de a página exibir conteúdos extras como vídeos, oferecendo uma demonstração dinâmica que complementa perfeitamente as imagens estáticas e o texto. Além disso, a presença de opções para compartilhar nas redes sociais facilita a divulgação orgânica do produto, aumentando seu alcance de forma prática.</p>

<p align="center">
  <img width="654" height="836" alt="devir6" src="https://github.com/user-attachments/assets/c07fcfbd-85b5-47e3-963f-44b9205e0778" />
  <br><em>Figura 6 - Vídeos do Produto 'Transgalactica'</em>
</p>


<p align="justify">No entanto, a experiência de usuário é prejudicada por algumas falhas significativas de organização e informação. Apesar de exibir as tags do produto, não há padrão e não há organização nelas, resultando em um sistema de classificação inconsistente e confuso, que pouco ajuda na descoberta de itens relacionados. O ponto negativo mais crítico, porém, é que a página não exibe preço ou locais para adquirir tal produto (qual loja física o contém). Esta omissão fundamental cria uma barreira direta entre a curiosidade gerada pelo conteúdo de qualidade e a conversão em uma venda, frustrando o usuário que, após se interessar pelo produto, não encontra uma maneira clara de adquiri-lo.</p>


### Conclusões:
<p align="justify">Os métodos de pesquisa escolhidos de fato representaram uma visão ampla do produto final a ser desenvolvido: tanto no contexto de funcionalidades para o site, quanto no contexto de quem é o usuário final.
Ressalta-se que, quanto mais direcionado e divulgado for o questionário, mais relevantes e assertivos são os dados obtidos (das 11 respostas obtidas, 3 são de pessoas que nunca compraram jogos e não demonstraram interesse no assunto).
Além disso, a criação dos campos de resposta para o usuário é fundamental para garantir a qualidade das informações obtidas. Como exemplo disto, uma das datas de nascimento obtidas foi errôneamente colocada no ano atual (representando o aniversário neste ano), tornando impossível a identificação da idade daquele registro.</p>

#### Stakeholders:
- Jogador de boardgames (cliente) - acessam o sistema para conferir promoções, novidades, fazer compras e acompanhar entregas.
- Dono de loja de jogos - acessam o sistema para anunciar os seus respectivos catálogos, obter contatos de possíveis clientes e receber os pagamentos
- Desenvolvedores do site - desenvolvem o sistema e acessam para testar as funcionalidades 

#### Descrições de tarefas:
<p align="justify">As tarefas a seguir descrevem os principais objetivos que cada um dos stakeholders identificados buscará alcançar. Elas abrangem tanto o uso da plataforma pelo cliente final quanto as atividades de gestão e manutenção do sistema.</p>

##### Tarefas do Cliente (Usuário Final)
- **Tarefa 1:** Encontrar um jogo para um grupo com gostos específicos
_Cenário:_ Fulana quer comprar um jogo de tabuleiro para sua noite de jogos semanal com 5 amigos. O grupo gosta de jogos cooperativos que não sejam muito longos (no máximo 90 minutos). Ela entra no site para filtrar as opções que se encaixem perfeitamente nessas condições, ler as descrições e decidir qual comprar.
- **Tarefa 2:** Descobrir se um jogo é adequado para iniciantes usando um leitor de telas
_Cenário:_ Fulano, que é cego e utiliza um software leitor de telas, ouviu falar bem de um jogo chamado "Ticket to Ride". Ele precisa que o site seja totalmente navegável via teclado para pesquisar por este jogo, ouvir a descrição do produto (alt text das imagens), entender o nível de dificuldade através das especificações e avaliações, e conseguir adicioná-lo ao carrinho e finalizar a compra de forma autônoma.

##### Tarefas do Lojista (Administrador)
- **Tarefa 3:** Anunciar um novo produto no site
_Cenário:_ O lojista acaba de receber um novo lote do aguardado lançamento "Frosthaven". Ele precisa acessar o painel de administração do site e, de forma rápida e intuitiva, cadastrar o novo produto, inserindo nome, descrição, fotos, preço, quantidade em estoque e especificações-chave (nº de jogadores, tempo, complexidade) para que ele apareça imediatamente na loja para os clientes.
- **Tarefa 4:** Analisar o desempenho de vendas do último mês
_Cenário:_ Ao final do mês, o lojista quer entender quais produtos foram os mais vendidos para planejar futuras compras com seus fornecedores. Ele acessa a seção de "Relatórios" no painel administrativo para gerar um resumo de vendas, visualizando um gráfico com os 10 jogos mais vendidos, o faturamento total e o número de pedidos realizados no período.

##### Tarefas do Desenvolvedor (Equipe Técnica)
- **Tarefa 5:** Implementar um novo filtro de busca solicitado pelo lojista
_Cenário:_ O lojista percebeu, através do feedback dos clientes, que muitos procuram jogos por "mecânica" (ex: "Alocação de Trabalhadores", "Construção de Baralho"). A equipe de desenvolvimento recebe a tarefa de modificar o sistema para adicionar este novo critério de filtro na página de busca, garantindo que ele funcione corretamente e não comprometa o desempenho do site.
- **Tarefa 6:** Corrigir um bug de acessibilidade reportado por um usuário
_Cenário:_ Um usuário reportou que o botão "Finalizar Compra" não está sendo lido corretamente pelos leitores de tela. O desenvolvedor precisa investigar o código-fonte da página de checkout, identificar a falha (provavelmente uma tag ARIA ausente ou incorreta), aplicar a correção, e publicar a atualização no site para garantir que o problema seja resolvido o mais rápido possível para todos os usuários.

#### E-Commerce

<p align="justify">Para ser eficaz e gerar confiança, uma loja online precisa ser extremamente descritiva em relação aos seus produtos. Isso vai além do simples nome e de uma imagem; é fundamental apresentar informações essenciais como preço, faixa etária, quantidade de peças e, em itens com partes pequenas, alertas claros sobre riscos de engasgo para crianças menores. Além de orientar a compra online, é um grande diferencial de atendimento ao cliente informar a loja física para adquirir o produto, caso o usuário prefira a experiência de compra presencial (fator muito importante para jogos de tabuleiro).</p>

<p align="justify">Também, a arquitetura do site precisa oferecer métodos de busca de produtos mais detalhados para que possa servir tanto a novatos quanto a entusiastas do hobby. Um iniciante pode se beneficiar de filtros intuitivos por categoria, preço e faixa etária, enquanto um colecionador experiente exigirá buscas avançadas por marca, número específico do produto, tema ou data de lançamento. Implementar um sistema de busca robusto e com múltiplas camadas de filtragem é, portanto, essencial para tornar a navegação eficiente e agradável para todos os perfis de público, transformando o site em uma ferramenta de descoberta e não em um obstáculo.</p>

<p align="justify">A apresentação dos produtos em forma de artigo é uma boa ideia, pois transforma uma simples listagem em uma narrativa envolvente (fator muito importante para artigos da linhagem geek). Esta abordagem permite explorar o contexto, a história e as emoções por trás de um item, utilizando imagens e textos descritivos que cativam o cliente e agregam valor percebido. No entanto, por mais atraente que seja, ela não pode ser o único detalhamento do produto. O entusiasmo gerado pelo artigo esbarra em uma parede se o consumidor, convencido da compra, não encontrar rapidamente as informações práticas necessárias para concluir a transação.</p>

<p align="justify">Dessa forma, combinar este tipo de apresentação com uma outra detalhada a respeito de preço, locais de venda etc é o ideal. Esta estrutura atende perfeitamente às duas mentalidades do comprador: a emocional, que é conquistada pelo storytelling do artigo, e a racional, que busca dados concretos para a decisão final. Um box ou seção bem visível ao lado ou ao final do artigo, contendo especificações técnicas, preço, disponibilidade em estoque e um seletor de lojas físicas, cria um fluxo de experiência completo e sem atritos. Desta forma, a plataforma consegue inspirar o usuário e, ao mesmo tempo, fornecer o caminho mais curto e eficiente para a conversão, unindo o melhor do marketing e do e-commerce.</p>

